...
% --------------------------------------------------------------------------------------------------
Const.FEKOmatfilename          = 'parabaloid.mat';
Const.FEKOstrfilename          = 'parabaloid.str';
Const.FEKOrhsfilename          = 'parabaloid.rhs';
Const.FEKOoutfilename          = 'parabaloid.out';
Const.FEKOefefilename          = 'parabaloid_NearField1.efe';
Const.FEKOffefilename          = 'parabaloid.ffe';
...
[SolutionPO] = runPOsolver(Const, Solver_setup, 0, 0, xVectors);
...
%             Solver_setup, Solution.mom.Isol);
        EfieldAtPointSphericalPO =  calculateEfieldAtPointRWG(Const, r, theta_degrees, phi_degrees, ...
            Solver_setup, SolutionPO.Isol);
...
%             abs(r.*EfieldAtPointSpherical(3))^2);
        Efield_magnitudePO(index) = sqrt(abs(r.*EfieldAtPointSphericalPO(1))^2 + ...
            abs(r.*EfieldAtPointSphericalPO(2))^2 + ...
            abs(r.*EfieldAtPointSphericalPO(3))^2);
    end%for
end%for
...
if (plot_normalised_field)
    ...
    max_Efield_magnitudePO = max(Efield_magnitudePO);
   ...
   % max_Efield_magnitude = 1.0*2*pi;
    max_Efield_magnitudePO = 1.0;
...
plot(1:total_efield_samples,Efield_magnitudePO./max_Efield_magnitudePO,'LineWidth',3);
...
legend( 'SUN-EM PO','FEKO (*.efe file)', 'FEKO (*.ffe file)');
