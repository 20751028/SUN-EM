function [po] = runPOsolver(Const, Solver_setup, zMatrices, yVectors, refIsol)
    ...
    %       Excitation source is set to 1 V/m x-polarised plane wave
...
%         Zmom = (calcZmn(Const, zMatrices, freq, 1, 1, ObservRWGs, SourceRWGs));
        ...
        for index=solStart:Npo
            ...
            k = 2*pi*500E6/3E8; %set 100MHz freq
            z = 120*pi;
            H = [0, (1/z)*exp(-j*k*(rn(3))), 0];    %Find impressed H field (z directed plane wave with E field x-polarised)
            %TODO: Generalise for any source and remove hard-coded plane wave
            po.Isol(index) = dot(2*delta*H, ln);
        end%for
        test = po.Isol*2*pi;
    ...
    % Compare the PO solution obtained with MATLAB, with that obtained by FEKO
    ...
    % Write the PO solution to a ASCII str file, so that it can be read
