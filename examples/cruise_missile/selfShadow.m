...
%offset for moving rays off origin
offset = min(Solver_setup.rwg_basis_functions_length_m(Solver_setup.num_mom_basis_functions), [], 'all')/10;
...
for ind = 1:Solver_setup.num_mom_basis_functions
    ...
    %fix rays intersecting with origin triangle by projecting the origin
    %slightly in the direction of departure.
    rays.origin = rays.origin + offset*(rays.dir./vecnorm(rays.dir, 2, 2));
    ...
    pp = (source_facing > 0.017) & (obs_facing < -0.017);
    pn = (source_facing > 0.017) & (obs_facing > 0.017);
    np = (source_facing < -0.017) & (obs_facing < -0.017);
    nn = (source_facing < -0.017) & (obs_facing > 0.017);
